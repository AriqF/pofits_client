...
import { ExpenseCategory, ExpenseTransactions, WalletData } from "@/utils/interfaces/server-props";
import { ExpenseForm } from "@/utils/interfaces/server-props";
import { IncomeForm } from "@/utils/interfaces/server-props";
...
export default function EditExpenseDataPage() {
  ...
  const setDefaultValue = async (data: ExpenseTransactions) => {
  ...
  const fetchData = async () => {
    try {
      const [currDataRes, categoryRes, walletRes] = await Promise.all([
        requestAxios({ url: baseUrl + "/transaction/expense/detail/" + dataId, method: "GET" }),
