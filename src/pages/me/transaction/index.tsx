...
import Backdrop from "@/components/tools/backdrop";
...
export default function TransactionIndex(props: Props) {
  ...
  const fetchTransactionsWithFilters = async () => {
    ...
    filterArr.push(`expenses=${toggleFilterExp}`);
    filterArr.push(`incomes=${toggleFilterInc}`);
    // if (searchQuery) filterArr.push(`search=${searchQuery}`);

    if (isRangeDate) {
      if (startDateFilter !== "")
        filterArr.push(`start_date=${moment(startDateFilter).format("YYYY-MM-DD")}`);
      if (endDateFilter !== "")
        filterArr.push(`end_date=${moment(endDateFilter).format("YYYY-MM-DD")}`);
    } else {
      if (startDateFilter) {
        if (startDateFilter !== "") {
          filterArr.push(`start_date=${moment(startDateFilter).format("YYYY-MM-DD")}`);
          filterArr.push(`end_date=${moment(startDateFilter).format("YYYY-MM-DD")}`);
        }
  ...
  useEffect(() => {
    fetchTransactions(`?expenses=true&incomes=true`);
    fetchRecapData();
  }, []);

  useEffect(() => {
    fetchTransactionsWithFilters();
  }, [searchQuery, startDateFilter, endDateFilter, toggleFilterExp, toggleFilterInc]);
  ...
  const FilterModal = () => {
    return (
      <div
        ...
        className={
          (showFilter
            ? "flex bg-gray-900 bg-opacity-50 fixed inset-0 overflow-y-hidden"
            : "hidden") + "  z-50 pt-10 md:pt-0  w-full items-center justify-center "
        }>
        ...
        <section className="col-span-4 gap-10 flex flex-col h-max-fit">
          ...
          <div id="transactions-history" className="flex flex-col gap-5">
            <div className="flex flex-col justify-between md:flex-row gap-y-5">
              <h2 className="text-2xl text-gray-800 font-semibold col-span-4 ">
                Riwayat Transaksi
              </h2>
              <a
